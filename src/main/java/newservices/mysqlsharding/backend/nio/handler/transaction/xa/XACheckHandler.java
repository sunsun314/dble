package newservices.mysqlsharding.backend.nio.handler.transaction.xa;
...
public final class XACheckHandler {
    ...
    public XACheckHandler(String xid, String schema, String shardingNode, PhysicalDbInstance ds) {
        this.xid = xid;
        this.ds = ds;
        this.schema = schema;
    ...
    private class CheckXidExistenceListener implements SQLQueryResultListener<SQLQueryResult<List<Map<String, String>>>> {
        ...
        public void onResult(SQLQueryResult<List<Map<String, String>>> result) {
            if (!result.isSuccess()) {
                // error
                LOGGER.warn("execute 'XA RECOVER' in " + ds.getDbGroup().getGroupName() + " error!");
                isSuccess = false;
