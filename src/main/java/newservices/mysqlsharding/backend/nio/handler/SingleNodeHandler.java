...
public class SingleNodeHandler implements ResponseHandler, LoadDataResponseHandler, ExecutableHandler {
    ...
    protected void execute(BackendConnection conn) {
        ...
        boolean isAutocommit = session.getService().isAutocommit() && !session.getService().isTxStart();
        if (!isAutocommit && node.isModifySQL()) {
            TxnLogHelper.putTxnLog(session.getService(), node.getStatement());
        }
        conn.execute(node, session.getService(), isAutocommit);
    }
    ...
    protected void backConnectionErr(ErrorPacket errPkg, BackendConnection conn, boolean syncFinished) {
        ...
        try {
            if (writeToClient.compareAndSet(false, true)) {
                if (rrs.isLoadData()) {
                    session.getService().getLoadDataInfileHandler().clear();
                }
                if (buffer != null) {
    ...
    protected void doSqlStat() {
        if (SystemConfig.getInstance().getUseSqlStat() == 1) {
            ...
            QueryResult queryResult = new QueryResult(session.getService().getUser(), rrs.getSqlType(), rrs.getStatement(), selectRows,
    ...
    public void fieldEofResponse(byte[] header, List<byte[]> fields, List<FieldPacket> fieldPacketsNull, byte[] eof,
        ...
        try {
            if (!writeToClient.get()) {
                buffer = session.getService().allocate();
        ...
        try {
            if (!writeToClient.get()) {
                ...
                if (session.isPrepared()) {
                    ...
                    buffer = binRowDataPk.write(buffer, session.getService(), true);
    ...
    public void connectionClose(BackendConnection conn, String reason) {
        ...
        err.setMessage(StringUtil.encode(reason, session.getService().getCharset().getResults()));
