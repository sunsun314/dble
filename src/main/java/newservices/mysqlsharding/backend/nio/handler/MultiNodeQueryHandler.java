...
import newservices.mysqlsharding.backend.LoadDataUtil;
import newservices.mysqlsharding.backend.nio.MySQLConnection;
import newservices.mysqlsharding.backend.nio.handler.transaction.AutoCommitHandler;
import newservices.mysqlsharding.backend.nio.handler.transaction.AutoTxOperation;
...
public class MultiNodeQueryHandler extends MultiNodeHandler implements LoadDataResponseHandler, ExecutableHandler {
    ...
    public MultiNodeQueryHandler(RouteResultset rrs, NonBlockingSession session) {
        ...
        if (ServerParse.SELECT == rrs.getSqlType()) {
            byteBuffer = session.getService().allocate();
        }
        this.sessionAutocommit = session.getService().isAutocommit();
    ...
    protected void reset() {
        super.reset();
        if (rrs.isLoadData()) {
            packetId = session.getService().getLoadDataInfileHandler().getLastPackId();
        }
        connRrns.clear();
        this.netOutBytes = 0;
        this.resultSize = 0;
    ...
    public void execute() throws Exception {
        ...
        if (sb.length() > 0) {
            TxnLogHelper.putTxnLog(session.getService(), sb.toString());
        }
        ...
        for (final RouteResultsetNode node : rrs.getNodes()) {
            ...
            if (session.tryExistsCon(conn, node)) {
                ...
                dn.getConnection(dn.getDatabase(), session.getService().isTxStart(), sessionAutocommit, node, this, node);
            }
        }
    }
    ...
    private void innerExecute(BackendConnection conn, RouteResultsetNode node) {
        ...
        mysqlCon.executeMultiNode(node, session.getService(), sessionAutocommit && !session.getService().isTxStart() && !node.isModifySQL());
    }

    @Override
    public void clearAfterFailExecute() {
        if (!session.getService().isAutocommit() || session.getService().isTxStart()) {
            session.getService().setTxInterrupt("ROLLBACK");
        }
    ...
    public void cleanBuffer() {
        if (recycledBuffer.compareAndSet(false, true)) {
            if (byteBuffer != null) {
                session.getService().recycle(byteBuffer);
            }
        }
    }
    ...
    public void connectionClose(BackendConnection conn, String reason) {
        ...
        errPacket.setMessage(StringUtil.encode(reason, session.getService().getCharset().getResults()));
        err = errPacket;
        ...
        try {
            ...
            if (decrementToZero(conn)) {
                ...
                if (session.closed()) {
                    ...
                    session.getService().write(byteBuffer);
                }
    ...
    public void rowEofResponse(final byte[] eof, boolean isLeft, BackendConnection conn) {
        ...
        RouteResultsetNode rNode = (RouteResultsetNode) conn.getAttachment();
        final ServerConnection source = session.getService();
        ...
        try {
            ...
            if (zeroReached) {
                this.resultSize += eof.length;
                if (!rrs.isCallStatement()) {
                    if (this.sessionAutocommit && !session.getService().isTxStart() && !session.getService().isLocked()) { // clear all connections
        ...
        try {
            ...
            if (!errorResponse.get()) {
                ...
                if (fconfig.isEnableFlowControl() &&
                        session.getService().getWriteQueue().size() > fconfig.getStart()) {
                    session.getService().startFlowControl(conn);
                }
                if (session.isPrepared()) {
                    ...
                    byteBuffer = binRowDataPk.write(byteBuffer, session.getService(), true);
                    this.packetId = (byte) session.getPacketId().get();
                } else {
                    if (row.length >= MySQLPacket.MAX_PACKET_SIZE + MySQLPacket.PACKET_HEADER_SIZE) {
                        byteBuffer = session.getService().writeBigPackageToBuffer(row, byteBuffer, packetId);
                        this.packetId = (byte) session.getPacketId().get();
                    } else {
                        row[3] = ++packetId;
                        byteBuffer = session.getService().writeToBuffer(row, byteBuffer);
                    }
                }
            }
        } catch (Exception e) {
    ...
    private void executeError(BackendConnection conn) {
        ...
        if (conn != null) {
            errConnection.add(conn);
            if (conn.isClosed() && (!session.getService().isAutocommit() || session.getService().isTxStart())) {
                session.getService().setTxInterrupt(error);
            }
        }
    ...
    void doSqlStat() {
        if (SystemConfig.getInstance().getUseSqlStat() == 1) {
            ...
            assert rrs != null;
            QueryResult queryResult = new QueryResult(session.getService().getUser(), rrs.getSqlType(),
