...
import newservices.mysqlsharding.backend.nio.handler.builder.sqlvisitor.MysqlVisitor;
    ...
    private static boolean isMatchSchema(String logicSchema, String table, String sourceSchema) {
        ...
        if (tbConfig == null) {
            ...
            for (String shardingNode : tbConfig.getShardingNodes()) {
                ShardingNode dbNode = DbleServer.getInstance().getConfig().getShardingNodes().get(shardingNode);
