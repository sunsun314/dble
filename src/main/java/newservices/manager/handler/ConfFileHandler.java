...
public final class ConfFileHandler {
    ...
    private static PackageBufINf upLoadConfigFile(ManagerService service,
        ...
        try {
            ...
            return showInfo(service, buffer, packetId, "write file err " + e);
        ...
        if (suc) {
            ...
            if (oldFile.exists()) {
                ...
                if (!oldFile.renameTo(backUP)) {
                    ...
                    return showInfo(service, buffer, packetId, msg);
                }
            }
            ...
            return showInfo(service, buffer, packetId, "SUCCESS SAVED FILE:" + fileName);
        } else {
            return showInfo(service, buffer, packetId, "UPLOAD ERROR OCCURD:" + fileName);
        }
    }

    private static PackageBufINf showInfo(ManagerService service,
                                          ByteBuffer buffer, byte packetId, String string) {
        RowDataPacket row = new RowDataPacket(FIELD_COUNT);
        row.add(StringUtil.encode(string, service.getCharset().getResults()));
    ...
    private static PackageBufINf showConfigFile(ManagerService service,
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                RowDataPacket row = new RowDataPacket(FIELD_COUNT);
                row.add(StringUtil.encode(line, service.getCharset().getResults()));
                row.setPacketId(++packetId);
                buffer = row.write(buffer, service, true);
            }
            bufINf.setBuffer(buffer);
    ...
    private static PackageBufINf listConfigFiles(ManagerService service,
