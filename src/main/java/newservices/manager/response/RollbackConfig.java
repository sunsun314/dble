...
public final class RollbackConfig {
    ...
    public static void execute(ManagerService service) {
        if (ClusterConfig.getInstance().isClusterEnable()) {
            if (rollbackWithCluster(service)) return;
        } else {
    ...
    private static boolean rollbackWithCluster(ManagerService service) {
        ...
        try {
            if (!distributeLock.acquire()) {
                service.writeErrMessage(ErrorCode.ER_YES, "Other instance is reloading/rollbacking, please try again later.");
                return true;
            }
            ClusterDelayProvider.delayAfterReloadLock();
            try {
                if (ClusterConfig.getInstance().isUseZK()) {
                    rollbackWithZk(ZKUtils.getConnection(), service);
                } else {
                    rollbackWithUcore(service);
                }
            } finally {
    ...
    private static void rollbackWithoutCluster(ManagerService service) {
        final ReentrantReadWriteLock lock = DbleServer.getInstance().getConfig().getLock();
        lock.writeLock().lock();
        try {
            if (!rollback(TRIGGER_TYPE_COMMAND)) {
                writeSpecialError(service, "Rollback interruputed by others,config should be reload");
            } else {
    ...
    private static void rollbackWithUcore(ManagerService service) {
    ...
    private static void rollbackWithZk(CuratorFramework zkConn, ManagerService service) {
        final ReentrantReadWriteLock lock = DbleServer.getInstance().getConfig().getLock();
        lock.writeLock().lock();
        try {
            if (!rollback(TRIGGER_TYPE_COMMAND)) {
            ...
            if (sbErrorInfo.length() == 0) {
                ...
                writeErrorResultForCluster(service, sbErrorInfo.toString());
            }
        } catch (Exception e) {
    ...
    private static void writeErrorResult(ManagerService service, String errorMsg) {
        String sb = "Rollback config failure.The reason is that " + errorMsg;
        LOGGER.info(sb + "." + String.valueOf(service));
        service.writeErrMessage(ErrorCode.ER_YES, sb);
    }
