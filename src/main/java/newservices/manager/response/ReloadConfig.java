...
public final class ReloadConfig {
    ...
    private static void execute(ManagerService service, final int loadAllMode) {
        if (ClusterConfig.getInstance().isClusterEnable()) {
            if (reloadWithCluster(service, loadAllMode)) return;
        } else {
            if (reloadWithoutCluster(service, loadAllMode)) return;
        }

        ReloadManager.reloadFinish();
    }

    private static boolean reloadWithCluster(ManagerService service, int loadAllMode) {
        ...
        try {
            ...
            try {
                ...
                if (ClusterConfig.getInstance().isUseZK()) {
                    reloadWithZookeeper(loadAllMode, ZKUtils.getConnection(), service);
                } else {
                    reloadWithUcore(loadAllMode, service);
                }
            } finally {
    ...
    private static boolean reloadWithoutCluster(ManagerService service, int loadAllMode) {
    ...
    private static void reloadWithUcore(final int loadAllMode, ManagerService service) {
    ...
    private static void reloadWithZookeeper(final int loadAllMode, CuratorFramework zkConn, ManagerService service) {
        final ReentrantReadWriteLock lock = DbleServer.getInstance().getConfig().getLock();
        lock.writeLock().lock();
        try {
            if (!reloadAll(loadAllMode)) {
