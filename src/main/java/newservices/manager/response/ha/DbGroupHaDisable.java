...
package newservices.manager.response.ha;
...
public final class DbGroupHaDisable {
        ...
        try {
            ...
            if (ClusterConfig.getInstance().isNeedSyncHa()) {
                if (ClusterConfig.getInstance().isUseZK()) {
                    if (!disableWithZK(id, dh, subHostName, service)) {
                        return;
                    }
                } else {
    ...
    private static boolean disableWithZK(int id, PhysicalDbGroup dh, String subHostName, ManagerService service) {
        ...
        if (!distributeLock.acquire()) {
            service.writeErrMessage(ErrorCode.ER_YES, "Other dble instance is change the dbGroup status");
        ...
        try {
            ...
            if (errorMessage != null && !"".equals(errorMessage)) {
                service.writeErrMessage(ErrorCode.ER_YES, errorMessage);
                return false;
            }
            return true;
        } catch (Exception e) {
    ...
    private static boolean disableWithCluster(int id, PhysicalDbGroup dh, String subHostName, ManagerService service) {
        ...
        try {
            if (!distributeLock.acquire()) {
                service.writeErrMessage(ErrorCode.ER_YES, "Other instance is changing the dbGroup, please try again later.");
