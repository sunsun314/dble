...
public final class DbGroupHaEnable {
        ...
        try {
            ...
            if (ClusterConfig.getInstance().isNeedSyncHa()) {
                if (ClusterConfig.getInstance().isUseZK()) {
                    if (!enableWithZK(id, dbGroup, dbInstanceName, service)) {
                        return;
                    }
                } else {
    ...
    private static boolean enableWithZK(int id, PhysicalDbGroup dh, String subHostName, ManagerService service) {
        ...
        if (!distributeLock.acquire()) {
            service.writeErrMessage(ErrorCode.ER_YES, "Other instance is change the dbGroup status");
            return false;
        }
        try {
    ...
    private static boolean enableWithCluster(int id, PhysicalDbGroup dh, String subHostName, ManagerService service) {
