...
public class SingleNodeDDLHandler extends SingleNodeHandler {
    ...
    public void execute() throws Exception {
        ...
        try {
            ...
            DDLTraceManager.getInstance().endDDL(session.getService(), e.getMessage());
            throw e;
        }
    }

    public void execute(BackendConnection conn) {
        DDLTraceManager.getInstance().updateConnectionStatus(session.getService(), (MySQLConnection) conn, DDLTraceInfo.DDLConnectionStatus.CONN_EXECUTE_START);
        super.execute(conn);
    }

    @Override
    public void connectionError(Throwable e, Object attachment) {
        DDLTraceManager.getInstance().updateRouteNodeStatus(session.getService(),
    ...
    public void errorResponse(byte[] data, BackendConnection conn) {
        DDLTraceManager.getInstance().updateConnectionStatus(session.getService(),
                (MySQLConnection) conn, DDLTraceInfo.DDLConnectionStatus.CONN_EXECUTE_ERROR);
        DDLTraceManager.getInstance().endDDL(session.getService(), "ddl end with execution failure");
    ...
    public void connectionClose(BackendConnection conn, String reason) {
        ...
        DDLTraceManager.getInstance().endDDL(session.getService(), reason);
    ...
    public void okResponse(byte[] data, BackendConnection conn) {
        ...
        if (executeResponse) {
            DDLTraceManager.getInstance().updateConnectionStatus(session.getService(), (MySQLConnection) conn, DDLTraceInfo.DDLConnectionStatus.CONN_EXECUTE_SUCCESS);
            // handleSpecial
            boolean metaInitial = session.handleSpecial(rrs, true, null);
            if (!metaInitial) {
                DDLTraceManager.getInstance().endDDL(session.getService(), "ddl end with meta failure");
                executeMetaDataFailed(conn);
            } else {
                DDLTraceManager.getInstance().endDDL(session.getService(), null);
    ...
    private void executeMetaDataFailed(BackendConnection conn) {
        ...
        doSqlStat();
        if (writeToClient.compareAndSet(false, true)) {
            errPacket.write(session.getService());
        }
        session.multiStatementNextSql(multiStatementFlag);
    }
