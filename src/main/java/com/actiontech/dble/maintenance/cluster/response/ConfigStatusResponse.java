...
import static com.actiontech.dble.sql.meta.ReloadStatus.TRIGGER_TYPE_CLUSTER;
...
public class ConfigStatusResponse implements ClusterXmlLoader {
    ...
    public void notifyProcess(KvBean pathValue) throws Exception {
        ...
        if (DbleServer.getInstance().getFrontProcessors() != null) {
            ...
            if ("".equals(ClusterHelper.getKV(ClusterPathUtil.getConfStatusPath()).getValue())) {
                LOGGER.info("return because response to an empty key");
                return;
            }
